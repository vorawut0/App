...
// Helper for simulated fallback responses
...
// AI Tutor Endpoint for Gemini API
app.post('/api/tutor', async (req: Request, res: Response) => {
  const { prompt = '', mode = 'study_tips', courseContext = null, messages = [] } = req.body;
...
Current Focus Course: ${courseContext?.title || 'All Active Courses'} (${courseContext?.code || 'General'})`;
  ...
  if (!apiKey || apiKey === 'MY_GEMINI_API_KEY') {
    const simulated = getSimulatedTutorResponse(prompt, mode, courseContext);
  ...
  try {
    ...
    res.json({ text: response.text || 'ไม่มีข้อมูลตอบกลับ' });
  } catch (error: any) {
    console.error('Tutor API Error:', error);
    const simulated = getSimulatedTutorResponse(prompt, mode, courseContext);
    res.json({ text: simulated, warning: error.message });
  }
});

// Serve Vite compiled static files from dist in production
...
app.listen(PORT, () => {
  console.log(`🚀 School Nexus Server running on http://0.0.0.0:${PORT}`);
});
